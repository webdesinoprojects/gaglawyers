...
const seedLocationPages = async () => {
  try {
    ...
    services.forEach((service, index) => {
      console.log(`   ${index + 1}. ${service.name || service.title} (${service._id})`);
    });
    console.log('');
    ...
    if (!CREATE_FOR_ALL_SERVICES) {
      console.log(`   Selected service: ${services[SELECTED_SERVICE_INDEX]?.name || services[SELECTED_SERVICE_INDEX]?.title || 'N/A'}`);
    }
    ...
    for (const service of servicesToProcess) {
      ...
      for (const city of locations) {
        ...
        const content = generateLocationContent(city, serviceName, service._id);
        const seo = generateSEO(city, serviceName, generateSlug(serviceName));
...
// Run the seeding (COMMENTED OUT - use fix-everything.js instead)
// seedLocationPages();
